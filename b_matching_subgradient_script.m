...
save_itn = 2;
%stochastic version or not 1:stochastic 2: no
is_stoch = 1;
%batch size
batch = 100;
%data directory
...
for f_ii=1:size(files,2)
    ...
    %         end
            ...
            if(is_stoch==1)
                s=zeros(n_v,1);
                rand_ind = randi(n_v,batch,1);
                s(rand_ind) = n_v*A(rand_ind,:)*x-b(rand_ind);
            else
                s = A*x-b;
            end
    ...
    %xlswrite(output_file,col_header,'sheet1','B1');
    Result
    clearvars -except data_source files f_ii L_it L_max B_set filetype graphtype T ubtype save_itn feas_need batch is_stoch
end
